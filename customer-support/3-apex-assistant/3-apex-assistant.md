...
1. Navigate to **Support Dashboard** page.
...
2. From the browser tab where the app is running, navigate to the runtime developer toolbar and click **Page 1**.
    ...
    !["Click App Builder"](images/edit-page1.png "")

3. In the page designer, select the first chart created under **Body**. In the property editor, update **Identification > Name** to **Case Status Summary**.

    !["Click App Builder"](images/case-status.png "")
...
5. Navigate to **APEX Assistant**. Select your query. From the menu, select **Query Builder** (if not already selected). In the APEX Assistant box, enter the following prompt and press enter:
    ...
    **Prompt 1:**
    ```
    <copy>
    Provide a breakdown of tickets by status using CS_TICKETS.
    </copy>
    ```
...
7. Click **Copy** or **Insert** to copy or insert the response into the Code Editor.
    ...
    !["Click App Builder"](images/code-insert.png "")
...
13. Navigate to **APEX Assistant** and enter the following prompt:
    ...
    **Prompt 1:**
    ```
    <copy>
    Show the number of tickets created each month over the last 12 months from CS_TICKETS.
    </copy>
    ```
...
2. Copy and paste the following *CREATE TABLE* statement in the code editor. You will then use APEX Assistant to generate a package that handles insert, update, and delete operations for the ticket updates table.
    ...
    *Note: To provide accurate context to the AI Assistant about which table it should reference while generating the package, we enter the following CREATE TABLE statement in the editor.*
    ...
    (
        ...
        "TICKET_ID" NUMBER NOT NULL
            CONSTRAINT cs_ticket_updates_ticket_fk REFERENCES cs_tickets,
...
3. Click **APEX Assistant**, select **General Assistance**, and enter the following prompt:
    ...
    **Prompt 1:**
     ```
    <copy>
    Generate a PL/SQL package to insert, update, and delete ticket updates from CS_TICKET_UPDATES.
    </copy>
    ```
...
8. Under **Packages**, you can see that the package has been created.
    ...
    *Note: This package can be extended further to improve the application. For example, you can add more procedures to handle escalation logic, fetch aggregated ticket details, or manage related data such as SLA records. Over time, this package can become the central place for all logic related to inserting, updating, and deleting ticket information in the application.*
